			...
			if (_this.options.enableLinks) {
				...
				treeItem
					.append($(_this.template.link)
						.attr('href', node.href)
						.append(!node.nodes ? node.oid + " : " + node.name + " (Type = " + node.datatype + " | Value = " + node.value + ")" : node.oid + " : " + node.name)
						//.append(node.oid + " : " + node.name )
					);
			}
			else {
				// otherwise just text
	...
	Tree.prototype.setAddNodes = function (node, options) {
	    ...
	    if (options.nodes) {
			...
			for (var i =0; i<node.nodes.length; i++){
				if(node.nodes[i]['oid'] == option.oid) return x=true;
			}
		    	if (x==false){
